...
class LSTMTagger_focus(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, labelset_size, pad_token_idx, pad_tag_idx, bidirectional=True, num_layers=1, dropout=0., device=None):
        """Initialize model."""
        ...
        #self.init_weights()
        self.intent_layer = intent.Intent(self.hidden_dim * self.num_directions, labelset_size, device)
    ...
    def forward(self, word_seqs, tag_seqs, lengths):
        ...
        intent, intent_scores = self.intent_layer(word_lstm_out)
        ...
        return tag_scores, (enc_h_t, enc_c_t), intent, intent_scores
    ...
    def decode_greed(self, word_seqs, init_tags, lengths):
        ...
        intents, intents_out = self.intent_layer(word_lstm_out)
        # decoder
        if self.bidirectional:
            index_slices = [2*i+1 for i in range(self.num_layers)]  # generated from the reversed path
            index_slices = torch.tensor(index_slices, dtype=torch.long, device=self.device)
        ...
        return top_path_tag_scores, top_path, (enc_h_t, enc_c_t),intents, intents_out
