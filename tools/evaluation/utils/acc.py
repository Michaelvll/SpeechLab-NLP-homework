...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--labfile', required=True,
                        help='path to dataset')
    parser.add_argument('-p', '--print_log',
                        action='store_true', help='print log')
    ...
    for idx in pred_lines:
        ...
        for pred_chunk in pred_chunks:
            if pred_chunk[-1] not in all_slots:
                all_slots[pred_chunk[-1]] = {'TP': 0.0,
                                             'FP': 0.0, 'FN': 0.0, 'TN': 0.0}
    ...
    if TP == 0:
        ...
        print('all slot', int(TP), int(FP), int(FN), round(100*TP/(TP+FP),
                                                           2), round(100*TP/(TP+FN), 2), round(100*2*TP/(2*TP+FN+FP), 2))
    ...
    for slot in all_slots:
        ...
        if TP == 0:
            ...
            print(slot, int(TP), int(FP), int(FN), round(100*TP/(TP+FP), 2),
                  round(100*TP/(TP+FN), 2), round(100*2*TP/(2*TP+FN+FP), 2))
