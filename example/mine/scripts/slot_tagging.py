...
label_vocab_dir = dataroot+'/lab_unali'
...
if not opt.testing:
    ...
    label_to_idx, idx_to_label_tag = data_reader.read_vocab_file(label_vocab_dir, bos_eos=opt.enc_dec)
...
if not opt.testing:
    train_feats, train_slot_tags, train_intents = data_reader.read_seqtag_data_with_unali_act(train_data_dir, word_to_idx, slot_tag_to_idx)
    valid_feats, valid_slot_tags, valid_intents = data_reader.read_seqtag_data_with_unali_act(valid_data_dir, word_to_idx, slot_tag_to_idx, keep_order=opt.testing)
    test_feats, test_slot_tags, test_intents = data_reader.read_seqtag_data_with_unali_act(test_data_dir, word_to_idx, slot_tag_to_idx, keep_order=opt.testing)
else:
...
if opt.task == 'slot_tagger':
    ...
    model_tag = slot_tagger_with_focus.LSTMTagger_focus(opt.emb_size, opt.hidden_size, len(word_to_idx), len(slot_tag_to_idx), len(label_to_idx), word_to_idx['<pad>'], slot_tag_to_idx['<pad>'], bidirectional=opt.bidirectional, num_layers=opt.num_layers, dropout=opt.dropout, device=opt.device)
elif opt.task == 'slot_tagger_with_crf':
    import models.slot_tagger_crf as slot_tagger_crf
    model_tag = slot_tagger_crf.LSTMTagger_CRF(opt.emb_size, opt.hidden_size, len(word_to_idx), len(slot_tag_to_idx), len(label_to_idx), word_to_idx['<pad>'], bidirectional=opt.bidirectional, num_layers=opt.num_layers, dropout=opt.dropout, device=opt.device)
...
intents_cnt={}
for intent in train_intents['data']:
    if intent not in intents_cnt:
        intents_cnt[intent]=1
    else:
        intents_cnt[intent]+=1
train_size = len(train_intents['data'])
for intent, cnt in intents_cnt.items():
    intents_cnt[intent] = cnt / train_size
for intent in label_to_idx:
    if intent not in intents_cnt:
        intents_cnt[intent]=1e-5
weight_mask =torch.tensor([1 / intents_cnt[idx_to_label_tag[idx]] for idx in range(len(idx_to_label_tag))], device=opt.device)
label_loss_function = nn.NLLLoss(weight=weight_mask)
...
if opt.optim.lower() == 'sgd':
    ...
    optimizer = optim.Adam(params, lr=opt.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-5) # (beta1, beta2)
...
def decode(data_feats, data_slot_tags, data_labels, output_path):
    ...
    with open(output_path, 'w') as f:
        for j in range(0, len(data_index), opt.test_batchSize):
            if opt.testing:
                ...
                inputs, slot_tags, lens, batch_labels = data_reader.get_minibatch_with_unali_act(data_feats, data_slot_tags, data_labels, word_to_idx, slot_tag_to_idx, label_to_idx, data_index, j, opt.test_batchSize, add_start_end=opt.bos_eos, keep_order=opt.testing, raw_word=False, enc_dec_focus=opt.enc_dec, device=opt.device)

            # slot tag
            if opt.enc_dec:
                opt.greed_decoding = True #True, False
                if opt.greed_decoding:
                    slot_tag_scores_1best, pred_slot_tag_1best, h_t_c_t, labels, labels_out = model_tag.decode_greed(inputs, slot_tags[:, 0:1], lens)
                    ...
                    label_loss = label_loss_function(labels_out, batch_labels)
                ...
                slot_tag_path_scores, slot_tag_path, labels, labels_out = model_tag.forward(crf_feats, masks)
                ...
                slot_tag_scores, h_t_c_t, labels, labels_out = model_tag(inputs, lens)
            ...
            label_amount += opt.test_batchSize
            label_cor += np.sum(batch_labels.data.cpu().numpy() == labels)

            losses.append(slot_tag_loss.item()/sum(lens) + alpha*label_loss.item())
            ...
            for idx, pred_line in enumerate(pred_slot_tag_1best):
                ...
                word_tag_line = [input_line[_idx]+':'+pred_seq[_idx] for _idx in range(len(input_line))]+[' <=> '+idx_to_label_tag[labels[idx]]]
                if opt.testing:
    ...
    label_acc = float(label_cor) / label_amount
    return mean_losses, p, r, f, label_acc
...
if not opt.testing:
    ...
    loss_val, p_val, r_val, f_val, label_acc_val = decode(valid_feats['data'], valid_slot_tags['data'], valid_intents['data'], os.path.join(exp_path, 'valid.iter'+str(-1)))
    logger.info('Validation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tLabelAcc : %.5f' % (-1, time.time() - start_time, loss_val, f_val, label_acc_val))
    start_time = time.time()
    loss_te, p_te, r_te, f_te, label_acc = decode(test_feats['data'], test_slot_tags['data'], test_intents['data'], os.path.join(exp_path, 'test.iter'+str(-1)))
    logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tLabelAcc : %.5f' % (-1, time.time() - start_time, loss_te, f_te, label_acc))
    for i in range(opt.max_epoch):
        ...
        for j in range(0, len(train_data_index), opt.batchSize):
            inputs, slot_tags, lens, labels = data_reader.get_minibatch_with_unali_act(train_feats['data'], train_slot_tags['data'], train_intents['data'], word_to_idx, slot_tag_to_idx, label_to_idx, train_data_index, j, opt.batchSize, add_start_end=opt.bos_eos, enc_dec_focus=opt.enc_dec, device=opt.device)
            optimizer.zero_grad()
            # slot tag
            if opt.enc_dec:
                slot_tag_scores, h_t_c_t, _, labels_out = model_tag(inputs, slot_tags[:, :-1], lens)
                slot_tag_loss = slot_tag_loss_function(slot_tag_scores.contiguous().view(-1, len(slot_tag_to_idx)), slot_tags[:, 1:].contiguous().view(-1))
                label_loss = label_loss_function(labels_out, labels)
            elif opt.crf:
                max_len = max(lens)
                masks = [([1] * l) + ([0] * (max_len - l)) for l in lens]
                masks = torch.tensor(masks, dtype=torch.uint8, device=opt.device)
                crf_feats, h_t_c_t = model_tag._get_lstm_features(inputs, lens)
                slot_tag_loss, label_loss = model_tag.neg_log_likelihood(crf_feats, masks, slot_tags, labels)
            else:
                slot_tag_scores, h_t_c_t, _, labels_out = model_tag(inputs, lens)
                slot_tag_loss = slot_tag_loss_function(slot_tag_scores.contiguous().view(-1, len(slot_tag_to_idx)), slot_tags.view(-1))
                label_loss = label_loss_function(labels_out, labels)
            losses.append(slot_tag_loss.item()/sum(lens) + alpha*label_loss.item())

            total_loss = slot_tag_loss/sum(lens) + alpha * label_loss
        ...
        model_tag.eval()
        start_time = time.time()
        loss_val, p_val, r_val, f_val, label_acc_val = decode(valid_feats['data'], valid_slot_tags['data'], valid_intents['data'], os.path.join(exp_path, 'valid.iter'+str(i)))
        logger.info('Validation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tLabelAcc : %.5f' % (i, time.time() - start_time, loss_val, f_val, label_acc_val))
        start_time = time.time()
        loss_te, p_te, r_te, f_te, label_acc = decode(test_feats['data'], test_slot_tags['data'], test_intents['data'], os.path.join(exp_path, 'test.iter'+str(i)))
        logger.info('Evaluation:\tEpoch : %d\tTime : %.4fs\tLoss : %.5f\tFscore : %.5f\tLabelAcc : %.5f' % (i, time.time() - start_time, loss_te, f_te, label_acc))

        if best_f1 < f_val:
            model_tag.save_model(os.path.join(exp_path, opt.save_model+'.tag'))
            best_f1 = f_val
            logger.info('NEW BEST:\tEpoch : %d\tbest valid F1 : %.5f\ttest F1 : %.5f\tvalid acc : %.5f' % (i, f_val, f_te, label_acc_val))
            best_result['iter'] = i
