...
def read_seqtag_data_with_unali_act(data_path, word2idx, slot_tag2idx, separator=':', keep_order=False, raw_word=False):
    ...
    with open(data_path, 'r') as f:
        for ind, line in enumerate(f):
            line_num += 1
            #print(ind)
            slot_tag_line, intent = line.strip().split(' <=> ')
            if slot_tag_line == "":
                continue
            in_seq, slot_tag_seq = [], []
            for item in slot_tag_line.split(' '):
    ...
    return input_feats, slot_tag_labels, intents

def get_minibatch_with_unali_act(input_seqs, slot_tag_seqs, labels, word2idx, slot_tag2idx, label2idx, train_data_indx, index, batch_size, add_start_end=False, keep_order=False, raw_word=False, enc_dec_focus=False, device=None):
    """Prepare minibatch."""
    ...
    labels = [labels[idx] for idx in train_data_indx[index:index + batch_size]]
    if add_start_end:
        if raw_word:
    ...
    labels = [label2idx[label] if label in label2idx else label2idx['<unk>'] for label in labels ]
    slot_tag_idxs = torch.tensor(slot_tag_idxs, dtype=torch.long, device=device)
    labels = torch.tensor(labels, dtype=torch.long, device=device)
    ret = [input_idxs, slot_tag_idxs, lens, labels]
    if keep_order:
